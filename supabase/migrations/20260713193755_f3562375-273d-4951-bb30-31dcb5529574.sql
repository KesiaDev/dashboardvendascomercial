...
BEGIN
  IF NEW.resolved = true AND NEW.state = 'aberto' THEN
    NEW.state := 'resolvido';
  ELSIF NEW.resolved = false AND NEW.state = 'resolvido' THEN
    NEW.state := 'aberto';
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_sync_alert_state
...
SELECT
  ...
  round(100.0 * sum(CASE WHEN ca.tentou_fechar THEN 1 ELSE 0 END)
        / NULLIF(count(*),0), 0)                       AS pct_fechamento
FROM public.coach_analyses ca
JOIN public.coach_conversations cc ON cc.id = ca.conversation_id
WHERE ca.status = 'ok'
  AND ca.analyzed_at >= now() - interval '12 weeks'
