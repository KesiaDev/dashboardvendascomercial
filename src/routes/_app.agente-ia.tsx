  ...
  head: () => ({
    meta: [
      { title: "Agente IA — Performance do SDR | LLMídia" },
      {
        name: "description",
        content:
          "Desempenho do agente comercial de IA: tempo de resposta, qualificação, conversão em reuniões e melhorias sugeridas.",
      },
      { property: "og:title", content: "Agente IA — Performance do SDR" },
      {
        property: "og:description",
        content: "Métricas de atendimento do agente IA da Clint no funil comercial.",
      },
    ...
    <Card className="border-warning/30">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Kpi
            icon={Bot}
            label="Vendas com a IA"
            value={String(vendas)}
            hint="Vendas com passagem pela IA"
          />
          <Kpi
            icon={Sparkles}
...
function Kpi({
  ...
  const toneCls =
    tone === "good" ? "text-success-fg" : tone === "warn" ? "text-warning-fg" : "text-foreground";
  return (
    <Card>
...
function AgenteIaPage() {
  ...
  useEffect(() => {
    supabase.auth
      .getSession()
      .then(({ data }) => setAdmin(isAdminUser(data.session?.user ?? null)));
  }, []);
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <Kpi
              ...
              value={String(k.conversasIa)}
            />

            <Kpi
              icon={TrendingUp}
              label="Taxa de resposta do lead"
              value={`${k.taxaRespostaPct}%`}
              tone={k.taxaRespostaPct >= 50 ? "good" : "warn"}
            />

            <Kpi
              icon={Clock}
              label="Tempo de 1ª resposta (mediana)"
            ...
            <Kpi
              icon={TrendingUp}
              label="Leads qualificados (3+ msgs)"
              value={String(k.qualificados)}
            ...
            <Kpi
              icon={Bot}
              label="Passou para humano"
              value={String(k.passouParaHumano)}
                ...
                return (
                  <>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      ...
                      <Kpi
                        icon={Trophy}
                        label="Vendas com passagem pela IA"
                        value={String(vendas)}
                        tone="good"
                      />
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      ...
                      <Kpi
                        ...
                        tone="good"
                        hint={`${realizadas} ÷ ${reunioes}`}
                      />
                      <Kpi
                        icon={Trophy}
                        label="Conv. sobre realizadas"
                        value={`${convRealizadas.toFixed(1)}%`}
                        tone={convRealizadas >= 25 ? "good" : "warn"}
                        hint={`${vendas} ÷ ${realizadas}`}
                      />
                      <Kpi
                        icon={TrendingUp}
                        label="Conv. sobre agendadas"
                        value={`${convAgendadas.toFixed(1)}%`}
                        tone={convAgendadas >= 20 ? "good" : "warn"}
                        hint={`${vendas} ÷ ${reunioes}`}
                      />
                    </div>
                  </>
                );
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">Funil do agente IA</CardTitle>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">Velocidade de resposta</CardTitle>
              </CardHeader>
              <CardContent className="h-64">
                <ResponsiveContainer width="100%" height="100%">
    ...
    return sessoes.filter(
      (s) =>
        ...
        (!q ||
          ...
          (s.ultimaMensagem ?? "").toLowerCase().includes(q)),
    );
              ...
              {filtradas.slice(0, limite).map((s) => (
                <tr key={s.id} className="border-t hover:bg-muted/30">
                  ...
                  <td className="px-3 py-2">
                    ...
                    {s.iaIniciou ? (
                      <span className="ml-1.5 text-[10px] text-muted-foreground">IA iniciou</span>
                    ) : null}
                  </td>
                  <td className="px-3 py-2">
                    <span
