...
function MinhaComissao() {
  ...
  if (data.rateMissing) {
    return (
      <Card className="border-destructive/50 bg-destructive/5">
        <CardHeader>
          <CardTitle className="text-base text-destructive">Cotação em falta</CardTitle>
        </CardHeader>
        <CardContent className="text-sm text-muted-foreground">
          {RATE_MISSING_MESSAGE} Fala com a gestora para a registar no período{" "}
          <strong>{data.period.nome}</strong>.
        </CardContent>
      </Card>
    );
  }
  ...
  const m = (brl: number) => money(moeda === "EUR" ? brl / (cotacao ?? Number.NaN) : brl, moeda);
