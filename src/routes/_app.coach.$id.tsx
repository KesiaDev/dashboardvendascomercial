...
function DetailPage() {
  ...
  const reanalyze = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["coach-conv", id] });
    },
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-4">
      <div className="flex items-center gap-2">
        ...
        <Button size="sm" onClick={() => reanalyze.mutate()} disabled={reanalyze.isPending}>
          <RefreshCw className={"h-4 w-4 mr-1 " + (reanalyze.isPending ? "animate-spin" : "")} />
          Reanalisar
        </Button>
      </div>
      ...
      <Card>
        <CardContent className="pt-4 space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {a?.sentimento && (
              <span className={"text-xs px-2 py-0.5 rounded " + sentimentColor(a.sentimento)}>
                {a.sentimento}
              </span>
            )}
          ...
          <p className="text-xs text-muted-foreground">
            Vendedor: {c?.seller_name ?? c?.seller_email ?? "—"} · {c?.origin_name ?? "—"} ·{" "}
            {c?.stage ?? "—"} · {msgs.length} msgs
          </p>
        </CardContent>
      </Card>
      ...
      {a?.status === "ok" && (
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              <CardContent className="pt-4 text-center">
                ...
                <p className={"text-5xl font-bold " + scoreColor(a.score_geral)}>
                  {Number(a.score_geral).toFixed(1)}
                </p>
              </CardContent>
            </Card>
            <Card>
            ...
            <Card>
              <CardContent className="pt-4">
                ...
                <p className="text-3xl font-bold mt-1">
                  {a.tempo_medio_resposta_min != null ? a.tempo_medio_resposta_min + " min" : "—"}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Tentou fechar: {a.tentou_fechar ? "sim" : "não"}
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-sm">{a.resumo ?? "—"}</p>
            </CardContent>
          </Card>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Objeções & oportunidades</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <div>
                  ...
                  {(a.objecoes ?? []).length === 0 ? (
                    ...
                    <ul className="list-disc pl-4 space-y-0.5">
                      {(a.objecoes ?? []).map((o: string, i: number) => (
                        <li key={i}>{o}</li>
                      ))}
                    </ul>
                  )}
                ...
                <div>
                  ...
                  {(a.oportunidades_perdidas ?? []).length === 0 ? (
                    <p className="text-xs text-muted-foreground">Nenhuma.</p>
                  ) : (
                    <ul className="list-disc pl-4 space-y-0.5">
                      {(a.oportunidades_perdidas ?? []).map((o: string, i: number) => (
                        <li key={i}>{o}</li>
                      ))}
                    </ul>
                  )}
            ...
            <CardContent className="space-y-3">
              ...
              {a.sugestao_resposta && (
                <div>
                  ...
                  <div className="mt-1 p-3 rounded bg-muted text-sm whitespace-pre-wrap">
                    {a.sugestao_resposta}
                  </div>
                </div>
              )}
              {(a.sugestoes ?? []).length > 0 && (
                <div>
                  ...
                  <ul className="list-disc pl-4 space-y-0.5 text-sm">
                    {(a.sugestoes ?? []).map((s: string, i: number) => (
                      <li key={i}>{s}</li>
                    ))}
                  </ul>
                </div>
            ...
            >
              ...
              >
                <p className="text-[10px] text-muted-foreground mb-0.5">
                  {m.sender_name ?? "—"} · {fmtDate(m.sent_at)}
                </p>
