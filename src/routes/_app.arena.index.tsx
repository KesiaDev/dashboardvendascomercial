...
const LEAGUE_COLOR: Record<string, string> = {
  ...
  Lenda:
    "bg-gradient-to-r from-amber-500/30 to-fuchsia-500/30 text-foreground border-fuchsia-500/50",
};
...
function ArenaDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Simulador profissional de vendas com clientes gerados por IA
          </p>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="pt-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              <Badge
                variant="outline"
                className={"text-sm px-3 py-1 " + (LEAGUE_COLOR[data.progress.league] ?? "")}
              >
                {data.progress.league}
              </Badge>
            </div>
        ...
        <Card>
          <CardContent className="pt-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {data.stats.total} simulações · {data.stats.winRate}% sucesso
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            {!mission && (
              ...
              >
                <RefreshCw
                  className={"h-4 w-4 mr-1 " + (generateMission.isPending ? "animate-spin" : "")}
                />
                Gerar missão
              </Button>
            )}
        ...
        <CardContent>
          {!spec ? (
            <p className="text-sm text-muted-foreground">
              Nenhuma missão gerada hoje. Clique em "Gerar missão" para começar.
            </p>
          ) : (
            <div className="space-y-3">
              <div className="grid gap-2 sm:grid-cols-3 text-sm">
                <div>
                  ...
                  <p className="font-semibold">{spec.produto}</p>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs">Canal</span>
                  <p className="font-semibold">{spec.canal}</p>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs">Perfil</span>
                  <p className="font-semibold">DISC {spec.perfil_disc}</p>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs">Dificuldade</span>
                  <p className="font-semibold">{spec.dificuldade}</p>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs">Objetivo</span>
                  <p className="font-semibold">{spec.objetivo}</p>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs">Recompensa</span>
                  <p className="font-semibold">{spec.recompensa_xp} XP</p>
                </div>
              </div>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Simulação livre</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Treine com um cliente aleatório na dificuldade escolhida. Cada persona é única.
          </p>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button onClick={() => startFree.mutate()} disabled={startFree.isPending}>
              <Play className="h-4 w-4 mr-1" />
              Iniciar simulação livre
            </Button>
            {data.openSim && (
              <Button
                variant="outline"
                onClick={() => navigate({ to: "/arena/sim/$id", params: { id: data.openSim!.id } })}
              >
      ...
      <div className="grid gap-4 md:grid-cols-2">
            ...
            {data.strongest.length === 0 ? (
              ...
              data.strongest.map((c: any) => (
                <div key={c.k} className="flex justify-between text-sm">
                  <span className="capitalize">{c.k.replace(/_/g, " ")}</span>
                  <span className="font-semibold text-success-fg">{c.avg.toFixed(1)}</span>
                </div>
              ))
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2">
              <TrendingDown className="h-4 w-4 text-destructive-fg" />A melhorar
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1.5">
            {data.weakest.length === 0 ? (
              ...
              data.weakest.map((c: any) => (
                <div key={c.k} className="flex justify-between text-sm">
                  <span className="capitalize">{c.k.replace(/_/g, " ")}</span>
                  <span className="font-semibold text-destructive-fg">{c.avg.toFixed(1)}</span>
                </div>
              ))
            )}
          </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {data.recent.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Nenhuma simulação ainda. Comece a treinar!
            </p>
          ) : (
                ...
                >
                  <span className="flex items-center gap-2">
                    <Badge
                      ...
                      className="text-[10px]"
                    >
                      {s.status === "open" ? "aberta" : (s.outcome ?? "finalizada")}
                    </Badge>
                  ...
                  <span className="flex items-center gap-3">
                    {s.score != null && (
                      <span className="font-bold">{Number(s.score).toFixed(0)}</span>
                    )}
                    {s.xp_earned > 0 && (
                      <span className="text-xs text-fuchsia-500">+{s.xp_earned} XP</span>
                    )}
                  </span>
                </button>
              ))}
