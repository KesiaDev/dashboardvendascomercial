...
export const fetchMyCommissionFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Sem cotação do período, `calculateCommissions` lança de propósito
    // (ver eur-rate.ts). Aqui isso viraria um 500 na cara do vendedor, então
    // devolvemos o motivo: número plausível e errado num ecrã de pagamento é
    // pior do que número nenhum.
    if (eurBrlRate(period) === null) {
      return {
