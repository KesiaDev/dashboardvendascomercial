...
/**
 * Núcleo, sem autenticação. Chamado tanto pelo server function autenticado
 * (browser) quanto pelas rotas de cron em api/public, que já validam o segredo
 * interno via requireApiKey. Sem esta separação, autenticar o server function
 * quebraria o cron — que roda no servidor e não tem Bearer token.
 */
export async function syncCcpbxCallsCore(data: { days?: number }) {
...
export const syncCcpbxCallsFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => syncCcpbxCallsCore(data));
 ...
 * quebraria o cron — que roda no servidor e não tem Bearer token.
 */
export async function analyzeCallCore(data: { callId: string }) {
...
export const analyzeCallFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => analyzeCallCore(data));
